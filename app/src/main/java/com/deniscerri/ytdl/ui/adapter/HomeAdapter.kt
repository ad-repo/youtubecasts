...
class HomeAdapter(onItemClickListener: OnItemClickListener, activity: Activity) : PagingDataAdapter<ResultItem, HomeAdapter.ViewHolder>(
    ...
    var onAddChannel: ((ResultItem) -> Unit)? = null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val addChannelBtn = card.findViewById<MaterialButton>(R.id.add_channel)
        if (onAddChannel != null && video.playlistURL.isPotentialYoutubeChannelUrl()) {
            addChannelBtn.visibility = View.VISIBLE
            addChannelBtn.setOnClickListener { onAddChannel?.invoke(video) }
        } else {
            addChannelBtn.visibility = View.GONE
            addChannelBtn.setOnClickListener(null)
        }
...
//        }
        if ((checkedItems.contains(video.id) && !inverted) || (!checkedItems.contains(video.id) && inverted)) {
            card.isChecked = true
            setSelectionStroke(card, true)
        } else {
            card.isChecked = false
            setSelectionStroke(card, false)
        }
    ...
    private fun setSelectionStroke(card: MaterialCardView, selected: Boolean) {
        card.strokeWidth = if (selected) 5 else 1
        card.strokeColor = if (selected) {
            MaterialColors.getColor(card, R.attr.colorPrimary)
        } else {
            ContextCompat.getColor(card.context, R.color.crisp_card_stroke)
        }
    }
    ...
    private fun String?.isPotentialYoutubeChannelUrl(): Boolean {
        if (isNullOrBlank()) return false
        val value = trim().lowercase()
        if (!value.contains("youtube.com/") && !value.contains("youtu.be/")) return false
        return value.contains("youtube.com/channel/") ||
            value.contains("youtube.com/c/") ||
            value.contains("youtube.com/user/") ||
            Regex("""youtube\.com/@[^/?#]+""").containsMatchIn(value)
    }
