...
class ChannelVideosFragment : Fragment(), HomeAdapter.OnItemClickListener {
    ...
    private var searchRenderJob: Job? = null
    ...
    private fun setupSearchView() {
        val searchItem = topAppBar.menu.findItem(R.id.action_search_channel_videos)
        ...
        searchView.queryHint = getString(R.string.search_channel_videos_hint)
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String): Boolean {
                ...
                renderVisibleLatest(debounceMs = 200)
                return true
            }
        })
    }

    private fun renderVisibleLatest(debounceMs: Long = 0) {
        searchRenderJob?.cancel()
        searchRenderJob = viewLifecycleOwner.lifecycleScope.launch {
            if (debounceMs > 0) delay(debounceMs)
            renderVisible()
        }
    }

    private fun filterVideos(videos: List<ResultItem>): List<ResultItem> {
        val query = searchQuery
        if (query.isBlank()) return videos
        return videos.filter { it.matchesChannelSearch(query) }
    }

    private fun ResultItem.matchesChannelSearch(query: String): Boolean {
        return title.contains(query, ignoreCase = true) ||
            author.contains(query, ignoreCase = true) ||
            url.contains(query, ignoreCase = true) ||
            playlistTitle.contains(query, ignoreCase = true) ||
            playlistURL.orEmpty().contains(query, ignoreCase = true) ||
            duration.contains(query, ignoreCase = true)
    }
