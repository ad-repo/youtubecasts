...
- Never push to, create branches on, or open pull requests against upstream (`deniscerri/ytdlnis`) unless the user explicitly requests upstream.
- Default all pushes and pull requests to the user's fork (`ad-repo/ytdlnis`).
- If a request says "push", "PR", or "create a PR" without naming a remote, use the fork only.
